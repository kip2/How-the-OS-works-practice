...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    draw_str_fg(&mut vram, 256, 256, 0xffffff, "Hello, WasabiOS!");
    let mut w = VramTextWritter::new(&mut vram);
    for i in 0..4 {
...
unsafe fn uncheked_draw_point<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64) {
...
fn fill_rect<T: Bitmap>(buf: &mut T, color: u32, px: i64, py: i64, w: i64, h: i64) -> Result<()> {
    ...
    for y in py..py + h {
        for x in px..px + w {
            unsafe {
                uncheked_draw_point(buf, color, x, y);
            }
        }
    }
    Ok(())
...
struct VramTextWritter<'a> {
    vram: &'a mut VramBufferInfo,
    cursor_x: i64,
    cursor_y: i64,
}

impl<'a> VramTextWritter<'a> {
...
impl fmt::Write for VramTextWritter<'_> {
